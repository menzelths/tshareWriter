...
var QreatorBezier=new function() {
	...
	for (var i = 0; i < farben.length; i++) {
		...
		var text= "<button class='farbe' farbe='" + farben[i]
				+ "' style='background: " + farben[i] + ";' value='Farbe' nr='"+i+"'><span style='color: "+farben[i]+"'>PP"+
				  "</span></button>";
		knoepfe+=text;
	}
	...
	this.init=function (selector,b,h){
		breite = b;
		 hoehe = h;
		selectorGlobal=selector;
		$(selector)
		.html(
				"<div id='rahmen' style='position: relative;'> <canvas id='layer0' width='"+breite+"' height='"+hoehe+"' style='z-index: 0;'></canvas> <canvas id='layer1'  width='"+breite+"' height='"+hoehe+"' style='position: absolute; left: 0; top: 0; z-index: 1;'></canvas><canvas id='layer2' width='"+breite+"' height='"+hoehe+"' style='position: absolute; left: 0; top: 0; z-index: 2;'></canvas> <canvas id='layer3'  width='"+breite+"' height='"+hoehe+"' style='position: absolute; left: 0; top: 0; z-index: 3;'></canvas></div><div>"
						+ knoepfe
						+ "</div><div id='knoepfe'> <input type='button' class='holen knopf' b='Stift' id='stift' value='Stift'/><input type='button' class='holen knopf' b='Radierer' id='radierer' value='Radierer'/><input type='button' class='holen knopf' b='Raster ändern' id='raster' value='Raster' /><input type='button' class='holen knopf' b='Alles löschen' id='loeschen' value='Alles löschen' /><input type='button' class='holen knopf' b='Aktive Ebene löschen' id='loeschenAktiv' value='Aktive Ebene löschen' /></div><div id='ausgabe'></div><div id='info' ></div>");
			...
			$(this).find("path").each(function(){
				...
				for (var j=0;j<dataChunks.length;j++){
					if (dataChunks[j]!=""){
						...
						var rest=teile[c].split(" ");
						if (rest.length>=3){
							for (var r=0;r<3;r++){
								koords=rest[r].split(",");
								p=new Punkt(parseFloat(koords[0])+punkte[0].x,parseFloat(koords[1])+punkte[0].y);
			punkte.push(p);
...
if (anzahlLayer<10){
	anzahlLayer=10;
		...
		zeichneEbenen(0,0,anzahlLayer-1);
	}
