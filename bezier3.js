...
var QreatorBezier=new function() {
	...
	menuGlobal,order, changes, changesPosition,orderPosition,unchanged;
...
gc.strokeStyle = "#ff0000";
	 ...
	 changesPosition=0; // gibt die position des ersten elements an, das nicht mehr betroffen ist
	 orderPosition=[]; // speichert für jede ebene die aktuelle position in der liste + 1 (zum löschen wichtig)
	...
	// layer 1 transparent machen
	 ...
	 order=[]; // speichert die zeichenreihenfolge in einer ebene: positiv wird gezeichnet, negativ nicht, liste startet bei 1 für ebene 0 (wegen vz)
	 changes=[]; // speichert alle aenderungen (außer raster): [Ebene, Typ (0 = von bis hinzu, 1 = von bis abziehen, 2 = von zu einzeln, 3 ebene löschen), daten ...]
	 ...
	 raster = false;
	 unchanged=true; // wenn seit dem letzten aufruf nicht gezeichnet wurde, dann true
	 maus = false;
	 touch = false;
	 debugging = 0;
	 gedrueckt = false;
	...
	$("#zurueck").click(function(event){
		...
		changesPosition--;
		if (changesPosition>=0){
			unchanged=true; // damit erkannt wird, dass seit dem letzten aufruf nichts gezeichnet wurde
		...
		var ebene=aenderung[j][0];
		 ...
		 // bereich
			var start=aenderung[j][1]; //beginn der transformation
			var ende=aenderung[j][2]; // ende der transformation
			for (var i=0;i<order[ebene].length; i++){
				if (order[ebene][i]>=start+1&& order[ebene][i]<=ende+1){ // nicht vergessen: order speichert um 1 erhöhte werte
	...
	$("#vor").click(function(event){
		if (unchanged==true&&changesPosition<changes.length){
		...
		var ebene=aenderung[j][0];
		 ...
		 // bereich
			...
			var ende=aenderung[j][2]; // ende der transformation
			for (var i=0;i<order[ebene].length; i++){
				if (Math.abs(order[ebene][i])>=start+1&& Math.abs(order[ebene][i])<=ende+1){ // nicht vergessen: order speichert um 1 erhöhte werte
					order[ebene][i]=-order[ebene][i];
					orderPosition[ebene][i]++;
				}
			}
		...
		if (xbreite.length > 0) {
			...
			for (var x = 0; x < breite; x += xbreite[xbreite.length - 1]) {
				for (var i = 0; i < xbreite.length; i++) {
					...
					kurven[ebene].push([gerade,Bezier.findeRechtecksHuelle(gerade),0,2,rasterfarbe]);
	...
	this.loadSVG=function(svg) { // converts svg-input to internal data format
		...
		$(svg).find("#global").find("g").each(function(){
			...
			punkte.push(p);
						...
						order[index][orderPosition[index]]=kurven[index].length; // reihenfolge speichern
						orderPosition[index]++;
			...
			if (kurven[index].length>0){
				//changes[changesPosition]=[index,0,0,kurven[index].length-1]; // alle ebenen speichern 
				//changesPosition++;
			}
		});
...
if (anzahlLayer<10){
	...
	function zeichneBezier(daten) {
		...
		changes[changesPosition]=[[layerfolge[layer],start,kurven[layerfolge[layer]].length-1]];
		changesPosition++;
		...
		for (var i=start;i<=stop;i++){
			...
			if (kurven[k]!=null){
				for (var j=0;j<orderPosition[k];j++){
					if (order[k][j]>0){
					var tmp=kurven[k][order[k][j]-1][0]; // 1 abziehen, da order immer um 1 zu hohen kurvenwert enthält
