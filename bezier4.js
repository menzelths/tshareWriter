var QreatorBezier = new function() {
	...
	var farben = [ "#000000", "#FF0000", "#0000FF", "#ffff00", "#008000",
			"#FFA500", "#A52A2A", "#800000", "#808000", "#808080", "#cccccc",
			"#FFFFFF", "#0000A0", "#ADD8E6", "#800080", "#00FF00", "#00FFFF",
			"#FF00FF" ];
	...
	for (var i = 0; i < layers.length; i++) {
		knoepfe += "<button class='layer' layer='" + layers[i] + "' >"
				+ layers[i]
				+ "</button><input type='checkbox' name='layer' id='checkbox_"
				+ i + "' value='" + i
				+ "' class='check' checked='checked'></input>";
	...
	this.init = function(selector, b, h, menuSelector, horizontal) {
		...
		$(menuSelector)
				.html(
						knoepfe
								+ "<div id='knoepfe'> <input type='button' class='holen knopf' b='Stift' id='stift' value='Stift'/><input type='button' class='holen knopf' b='Radierer' id='radierer' value='Radierer'/><input type='button' class='holen knopf'  id='zurueck' value='Zurück' /><input type='button' class='holen knopf'  id='vor' value='Vor' /><input type='button' class='holen knopf' b='Raster ändern' id='raster' value='Raster' /><input type='button' class='holen knopf' b='Alles löschen' id='loeschen' value='Alles löschen' /><input type='button' class='holen knopf' b='Aktive Ebene löschen' id='loeschenAktiv' value='Aktive Ebene löschen' /></div><div id='ausgabe'></div><div id='info' ></div>");
									...
									for (var j = 0; j < aenderung.length; j++) {
										if (aenderung[j].length == 3) { // bereich
											...
											for (var i = 0; i < order[ebene].length; i++) {
												if (order[ebene][i] >= start + 1
														&& order[ebene][i] <= ende + 1) { // nicht
													// vergessen:
													// order
													// speichert
													// um 1
										...
										} else if (aenderung[j].length == 2) { // einzelne
											var ebene = aenderung[j][0];
											var nr = aenderung[j][1]; // aktuelles
																		// objekt
							...
							if (unchanged == true
								...
								for (var j = 0; j < aenderung.length; j++) {
									...
									var ende = aenderung[j][2]; // ende der
																// transformation
									for (var i = 0; i < order[ebene].length; i++) {
										if (Math.abs(order[ebene][i]) >= start + 1
												&& Math.abs(order[ebene][i]) <= ende + 1) { // nicht
											// vergessen:
											// order
											// speichert
											// um 1
				...
				for (var i = 0; i < kurven[k].length; i++) {
					...
					if (text.length > 2000) {
						...
						if (svgtext.length > 10000) {
							$("#qreator_svgbild2 #global #layer_" + k).append(
									svgtext);
	...
	function zeichneRaster(xbreite, ybreite, rasterfarbe) {
		...
		zeichneEbenen(layerfolge[layer], ebene, layerfolge[layer]);
														...
														for (var c = 1; c < teile.length; c++) {
															...
															kurven[index]
																	.push([
																			...
																			breiteLoad,
																			farbeLoad ]);
															order[index][orderPosition[index]] = kurven[index].length; // reihenfolge
															// speichern
															orderPosition[index]++;
	...
	function zeichneOderRadiere() {
		...
		if (berechnung === true) {
			...
			if (zeichenmodus === 0) {
				...
				for (var i = 0; i < layerfolgen.length; i++) {
					betroffeneEbenen[i] = 0;
					if (i == layerfolgen[layer]) {
					...
					if (betroffeneEbenen[kurvenebene] > 0) {
						if (kurven[kurvenebene] != null
							...
							for (var i = betroffeneKurven.length - 1; i >= 0; i--) { // alle
								...
								// kurven
								// durchlaufen
								var tmp = kurven[kurvenebene][order[betroffeneKurven[i]] - 1][0];
								var stiftdicke_aktuell = kurven[kurvenebene][order[betroffeneKurven[i]] - 1][3];
								var farbe_aktuell = kurven[kurvenebene][betroffeneKurven[order[betroffeneKurven[i]] - 1]][4];
								...
								if (zaehler - test < 3) { // falls nur 2
									...
									aenderungen
											.push([
													kurvenebene,
													-order[kurvenebene][betroffeneKurven[i]] - 1 ]); // löschung
									// registrieren
									...
									if (tk.length > 0) {
										...
										aenderungen
												.push([
														kurvenebene,
														start,
														kurven[kurvenebene].length - 1 ]);
									}

								}
							}

							changes[changePosition] = aenderungen;
							changesPosition++;
							daten = [];
...
function loescheZeichnung() {
	...
	if (rechtecke2.length < grenzeAnzahl) { // ecken von dem einen rechteck
		...
		gc.putImageData(leer, 0, 0, ecken[0], ecken[1], ecken[2] - ecken[0],
				ecken[3] - ecken[1]);
...
function zeichneKurvenNeu() {
	...
	if (rechtecke2.length < grenzeAnzahl) { // ecken von dem einen rechteck
		...
		gc2.clip();
		gc2.putImageData(leer, 0, 0, ecken[0], ecken[1], ecken[2] - ecken[0],
				ecken[3] - ecken[1]);
	} else { // sonst rechtecke klippen
		gc2.beginPath();
		var b = breite / rx;
	...
	for (var i = 0; i < kurven[kurvenebene].length; i++) {
		...
		if (tmp1[2] == 1) { // nur dann neu zeichnen
			...
			gc2.bezierCurveTo(tmp[1].x, tmp[1].y, tmp[2].x, tmp[2].y, tmp[3].x,
					tmp[3].y);
...
function zeichneBezier(daten) {
	...
	for (var i = 0; i < erg.length; i++) {
		...
		if (tmp.length === 1) { // punkt
			...
			gc2.stroke();
			kurven[layerfolge[layer]]
					.push([ tmp, Bezier.findeRechtecksHuelle(tmp), 0,
							zeichenbreite, farbe ]); // alle

			order[layerfolge[layer]][orderPosition[layerfolge[layer]]] = kurven[layerfolge[layer]].length;
	...
	changes[changesPosition] = [ [ layerfolge[layer], start,
			kurven[layerfolge[layer]].length - 1 ] ];
	changesPosition++;
...
function zeichneEbenen(centralLayer, start, stop) {
	...
	for (var i = start; i <= stop; i++) {
		...
		if (visibleLayers[i] == 1) { // nur zeichnen, wenn sichtbar
			if (kurven[k] != null) {
				for (var j = 0; j < orderPosition[k]; j++) {
					if (order[k][j] > 0) {
...
var Bezier = {
	...
	findeKonvexeHuelle : function(points) { // liefert die konvexe hülle der
		...
		function left_oriented(p1, p2, candidate) {
			...
			return berechneEntfernung(p1, candidate) > berechneEntfernung(p1,
					p2);
		}
		var N = points.length;
		var hull = [];
	...
	approximiereKubisch : function(start, ende, tan1, tan2) {
		...
		// wiederholen
		var V1 = this.subtrahiere(this.punkte[split - 1], this.punkte[split]), V2 = this
				.subtrahiere(this.punkte[split], this.punkte[split + 1]), tanCenter = this
				.normiere(this
						.multipliziereMitSkalar(this.addiere(V1, V2), 0.5));
	...
	findeBezierNaeherung : function(start, ende, param_u, tan1, tan2) { // findet
		...
		for (var i = 0; i < anzahl; i++) { // die nachfolgenden zeilen
			...
			b0 = t * t * t, b1 = b * t, b2 = b * u, b3 = u * u * u, a1 = this
					...
					.subtrahiere(this.subtrahiere(this.punkte[start + i], this
							.multipliziereMitSkalar(pkt1, b0 + b1)), this
							.multipliziereMitSkalar(pkt2, b2 + b3));
	...
	evaluiere : function(grad, kontrollpunkte, t) { // berechnet den
		...
		for (var i = 1; i <= grad; i++) {
			for (var j = 0; j <= grad - i; j++) {
				tmp[j] = this.addiere(this
						.multipliziereMitSkalar(tmp[j], 1 - t), this
						.multipliziereMitSkalar(tmp[j + 1], t));
			}
		}
		return tmp[0];
	},
	...
	reparametrisiere : function(start, ende, u, kontrollpunkte) { // ruft
		...
		for (var i = start; i <= ende; i++) {
			u[i - start] = this.findeNullstelle(kontrollpunkte, this.punkte[i],
					u[i - start]);
		}
		return u;
	},
	findeNullstelle : function(kontrollpunkte, punkt, u) {
		...
		var pt = this.evaluiere(3, kontrollpunkte, u), pt1 = this.evaluiere(2,
				k1, u), pt2 = this.evaluiere(1, k2, u), diff = this
				.subtrahiere(pt, punkt), df = this
				.multipliziereSkalar(pt1, pt1)
